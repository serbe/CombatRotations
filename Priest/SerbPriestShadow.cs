...
//			}
		...
		public bool Desicion() {
			...
			if (HasSpell("Clarity of Power") && !HasSpell("Insanity"))
				COP();
			// actions.decision+=/call_action_list,name=cop_dotweave,if=talent.clarity_of_power.enabled&talent.insanity.enabled&target.health.pct>20&active_enemies<=6
			if (HasSpell("Clarity of Power") && HasSpell("Insanity") && TargetHealth > 0.2 && EnemyInRange(40) <= 6)
				COP_DotWeave();
			// actions.decision+=/call_action_list,name=cop_insanity,if=talent.clarity_of_power.enabled&talent.insanity.enabled
			if (HasSpell("Clarity of Power") && HasSpell("Insanity"))
				COP_Insanity();
		...
		public boo Main() {
			...
			if (Cast("Mind Blast", () => HasGlyph(162532) && Orb <= 2 && EnemyInRange(40) <= 5 && Cooldown("Mind Blast") == 0)) return;
			...
			if (Cast("Devouring Plague", () => Orb >= 3 && Me.AuraTimeRemaining("Mental Instinct") < 1.5 && Me.AuraTimeRemaining("Mental Instinct") > 1.5 * 0.7) && Me.HasAura("Mental Instinct")) return;
			...
			if (Orb >= 4 && HasSpell("Auspicious Spirits")) {
				CycleTarget = targets.Where(u => u.IsInCombatRangeAndLoS && ((Cooldown("Mind Blast") < 1.5 && !HasSpell("Mental Instinct")) || (u.HealthFraction < 0.2 && Cooldown("Shadow Word: Death") < 1.5)) && !u.HasAura("Devouring Plague", true) && HasSpell("Surge of Darkness")).DefaultIfEmpty(null).FirstOrDefault();
				if (Cast("Devouring Plague", CycleTarget, () => CycleTarget != null)) return;
			}
			...
			if (Cast("Devouring Plague", () => Orb >= 4 && HasSpell("Auspicious Spirits") && ((Cooldown("Mind Blast") < 1.5 && !HasSpell("Mental Instinct")) || (TargetHealth < 0.2 && Cooldown("Shadow Word Death") < 1.5)))) return;
			...
			if (Cast("Devouring Plague", () => Orb >= 3 && !HasSpell("Auspicious Spirits") && ((Cooldown("Mind Blast") < 1.5 && !HasSpell("Mental Instinct")) || (TargetHealth < 0.2 && Cooldown("Shadow Word Death") < 1.5)))) return;
			...
			if (Cast("Mind Blast", () => HasSpell("Auspicious Spirits") && EnemyInRange(40) <= 4 && Cooldown("Mind Blast") == 0)) return;
	...
	//			get {
	//				int CountOfShadowApparitions = API.Units.Where(u => u.EntryID == 46954).ToList().Count;
	...
	//
	//			if (ShadowApparitions > 0 && !HasGlobalCooldown()) {
