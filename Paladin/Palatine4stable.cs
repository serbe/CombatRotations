...
namespace ReBot
{
	[Rotation ("Paladine4 stable", "McFly", "Divine Knight", WoWClass.Paladin, Specialization.PaladinHoly, 40)]
	public class Paladine4_stable : CombatRotation
	{
		...
		public override bool OutOfCombat ()
		{
			...
			var lowestPlayer = group.Where (p => p.HealthFraction > 0 && !p.IsDead).OrderBy (p => p.HealthFraction).DefaultIfEmpty (null).FirstOrDefault ();
		...
		{
			bool done = false;
			var lowestPlayer = group.Where (p => p.HealthFraction > 0 && !p.IsDead).Where (p => !p.HasAura (myHealTalent1)).OrderBy (p => p.HealthFraction).DefaultIfEmpty (null).FirstOrDefault ();
			if (HP > 3 && lowestPlayer != null) {
		...
		public override void Combat ()
		{
			...
			var lowestPlayer = grpAndMe.Where (p => p.HealthFraction > 0 && !p.IsDead).OrderBy (p => p.HealthFraction).DefaultIfEmpty (null).FirstOrDefault ();
