...
namespace ReBot.DeathKnight
{
	...
	public class SerbDeathKnightBloodSc : SerbDeathKnight
	{
		...
		bool ActionsBt ()
		{
			...
			if (GetAuraStack ("Blood Charge", Me) >= 5 && !HasBlood)
				BloodTap ();
			//	actions.bt+=/death_strike,if=buff.blood_charge.stack>=10&unholy&frost
			if (GetAuraStack ("Blood Charge", Me) >= 10 && HasUnholy && HasFrost) {
				if (DeathStrike ())
					return true;
			}
			...
			if (GetAuraStack ("Blood Charge", Me) >= 5 && (!HasUnholy || !HasFrost))
				BloodTap ();
			//	actions.bt+=/blood_tap,if=buff.blood_charge.stack>=5&blood.death&!unholy&!frost
			if (GetAuraStack ("Blood Charge", Me) >= 5 && HasDeath && !HasUnholy && !HasFrost)
				BloodTap ();
